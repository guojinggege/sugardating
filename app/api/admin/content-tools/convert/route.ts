// POST /api/admin/content-tools/convert · 启发式笔记 → Journal 结构化输出
import { NextResponse } from "next/server";
import { requireAdminOrErr } from "@/lib/admin/auth";
import { convertNoteToBlog } from "@/lib/content-tools/note-to-blog";
import { saveConversion } from "@/lib/content-tools/conversion-store";
import { recordAudit } from "@/lib/cms/audit";
import type { ConversionInput, ConversionSettings, ConversionLanguage, ConversionTone, ConversionLength } from "@/lib/content-tools/conversion-types";
...
export async function POST(req: Request) {
  ...
  if (!rawText || rawText.length < 30) {
    return NextResponse.json({ ok: false, message: "正文过短 · 至少 30 字" }, { status: 400 });
  }
  if (rawText.length > 20000) {
    return NextResponse.json({ ok: false, message: "正文过长 · 请分段处理" }, { status: 400 });
  }
  ...
  try {
    ...
    recordAudit({
      ...
      action: "create", targetType: "content-conversion", targetId: output.id,
      summary: `笔记转换 · ${output.title.slice(0, 50)}`,
    });
