    ...
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-feed-line bg-feed-surface transition duration-300 ease-out hover:-translate-y-1 hover:border-gold/40 hover:shadow-[0_24px_60px_-20px_rgba(0,0,0,0.55)]">
      ...
      <Link href="/membership" className="relative block overflow-hidden" style={{ aspectRatio: "3/4" }}>
        ...
        {entry.tags.length > 0 && (
          <div className="absolute bottom-3 left-3 flex flex-wrap gap-1.5">
            {entry.tags.map((t) => (
              <span key={t} className={`rounded-full px-2 py-0.5 text-[10.5px] font-bold tracking-wider backdrop-blur ${TAG_STYLE[t]}`}>
                {t}
              </span>
            ))}
          </div>
      ...
      <div className="flex flex-1 flex-col gap-2 p-4">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <div className="shrink-0 text-[12px] text-feed-dim">{entry.city}</div>
        </div>
        <p className="line-clamp-2 text-[12.5px] leading-relaxed text-feed-mute">{entry.bio}</p>

        <div className="mt-2 flex items-center justify-between gap-2">
          <span className="text-[11px] text-feed-dim tabular-nums">{entry.height} cm</span>
          <Link
            href="/membership"
